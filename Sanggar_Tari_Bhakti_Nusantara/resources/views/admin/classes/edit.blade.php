        ...
        <div class="bg-white border border-[#FEDA60]/30 shadow-lg rounded-2xl p-6">
            <form action="{{ route('classes.update', $class) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    @php
                        $names = collect(explode(',', (string)$class->instructor))->map(function($n){ return trim($n); })->filter();
                        $currentInstructorIds = collect($teachers ?? [])->filter(function($t) use ($names) {
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                        @foreach(($teachers ?? []) as $t)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="instructor_ids[]" value="{{ $t->id }}" {{ in_array($t->id, (array)old('instructor_ids', $currentInstructorIds)) ? 'checked' : '' }}
                                    class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
