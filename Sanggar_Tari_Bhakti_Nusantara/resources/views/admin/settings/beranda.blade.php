    ...
    <div class="flex">
        ...
        <main class="flex-1 lg:ml-0 p-6 lg:p-10">
            <div class="max-w-6xl mx-auto">
                <!-- Header -->
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-[#2E2E2E] mb-2">Pengaturan Beranda</h1>
                    <p class="text-[#4F4F4F]">Kelola konten di halaman beranda</p>
                </div>
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-[#E6D8A1]/40 p-8 mb-6">
                    ...
                    <form action="{{ route('admin.settings.beranda.update') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Since Year -->
                            <div>
                                <label for="since_year" class="block text-sm font-semibold text-[#2E2E2E] mb-2">
                                ...
                                <input 
                                    ...
                                    id="since_year" 
                                    name="since_year" 
                                    value="{{ old('since_year', $sinceYear) }}"
                            ...
                            <!-- Since Text -->
                            <div>
                                <label for="since_text" class="block text-sm font-semibold text-[#2E2E2E] mb-2">
                                    Text Deskripsi <span class="text-red-500">*</span>
                                </label>
                                <input 
                                    type="text" 
                                    id="since_text" 
                                    name="since_text" 
                                    value="{{ old('since_text', $sinceText) }}"
                                    class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-[#F5B347] focus:ring-2 focus:ring-[#F5B347]/20 outline-none transition-all text-gray-900 placeholder-gray-400"
                                    placeholder="Melayani Pendidikan Seni Tari"
                                    required
                                >
                                @error('since_text')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-[#E6D8A1]/40 p-8">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <a href="{{ route('admin.gallery.image.create') }}" class="px-4 py-2 bg-[#2E2E2E] text-white rounded-xl hover:bg-[#3E3E3E] transition-all">
                            + Tambah Gambar
                        </a>
                    </div>

                    <p class="text-sm text-[#4F4F4F] mb-6">
                        Card galeri kegiatan dikelola di menu <a href="{{ route('admin.gallery.image.index') }}" class="text-[#F5B347] font-semibold hover:underline">Galeri Media → Gambar Galeri</a>. 
                        Gunakan kategori "galeri_kegiatan" untuk menampilkan di beranda.
                    </p>
                        ...
                        @forelse($galleryImages as $image)
                            <div class="relative group rounded-xl overflow-hidden border-2 border-gray-200 hover:border-[#F5B347] transition-all">
                                ...
                                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.gallery.image.edit', $image) }}" class="px-3 py-2 bg-[#FEDA60] text-[#2E2E2E] rounded-lg hover:bg-[#F5B347] transition-all text-sm font-semibold">
                                        Edit
                                    </a>
                                </div>
                                <div class="p-3 bg-white">
                                    <p class="font-semibold text-[#2E2E2E] text-sm truncate">{{ $image->title }}</p>
                                    <p class="text-xs text-[#4F4F4F] mt-1">{{ $image->category }}</p>
                                </div>
                            </div>
                        @empty
                            <div class="col-span-3 text-center py-12 text-[#4F4F4F]">
                                ...
                                <p class="mb-4">Belum ada gambar galeri kegiatan</p>
                                <a href="{{ route('admin.gallery.image.create') }}" class="inline-block px-6 py-3 bg-gradient-to-r from-[#FEDA60] to-[#F5B347] text-[#2E2E2E] font-semibold rounded-xl hover:shadow-lg transition-all">
