...
<div class="min-h-screen bg-gradient-to-br from-[#F5F5F5] via-[#FFF9E5] to-[#FFF6D5]">
    ...
    <div class="max-w-3xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-[#2E2E2E]">Tambah Carousel Baru</h1>
            <p class="text-[#4F4F4F] mt-2">Isi form di bawah untuk menambahkan carousel</p>
        </div>
        ...
        <form action="{{ route('admin.gallery.carousel.store') }}" method="POST" enctype="multipart/form-data" class="rounded-2xl bg-white border border-[#FEDA60]/30 p-8 shadow-lg">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" required
                        class="w-full px-4 py-3 rounded-xl border @error('description') border-red-500 @else border-gray-300 @enderror focus:border-[#FEDA60] focus:ring-2 focus:ring-[#FEDA60]/20 text-gray-900 placeholder-gray-400 transition-all">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="image" class="block text-sm font-semibold text-[#2E2E2E] mb-2">Gambar <span class="text-red-500">*</span></label>
