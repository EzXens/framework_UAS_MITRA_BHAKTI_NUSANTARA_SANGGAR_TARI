    ...
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('admin.gallery.video.update', $video) }}" method="POST" enctype="multipart/form-data" class="rounded-2xl bg-white border border-[#FEDA60]/30 p-8 shadow-lg space-y-6">
            ...
            <div>
                ...
                <input type="url" name="video_url" value="{{ old('video_url', $video->video_url) }}" placeholder="https://www.youtube.com/embed/VIDEO_ID?autoplay=1&rel=0" class="w-full px-4 py-3 rounded-xl border @error('video_url') border-red-500 @else border-gray-300 @enderror focus:border-[#FEDA60] focus:ring-2 focus:ring-[#FEDA60]/20 text-gray-900 placeholder-gray-400">
                @error('video_url')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
