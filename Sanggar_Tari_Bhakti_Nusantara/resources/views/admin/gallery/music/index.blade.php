            ...
            @forelse($music as $track)
                <div class="rounded-2xl bg-white border border-[#FEDA60]/30 p-6 shadow-lg hover:shadow-xl transition-all">
                    <div class="flex items-center gap-6">
                        <div class="flex-shrink-0 w-20 h-20 rounded-xl bg-gradient-to-br from-[#FEDA60] to-[#F5B347] flex items-center justify-center">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between mb-2">
                                <div>
                                    <h3 class="text-xl font-bold text-[#2E2E2E]">{{ $track->title }}</h3>
                                    <p class="text-sm text-[#4F4F4F] mt-1">{{ $track->description }}</p>
                                    <p class="text-xs text-gray-500 mt-2">File: {{ basename($track->audio_file) }}</p>
                                </div>
                            ...
                            <audio controls preload="none" class="w-full max-w-md mt-3">
                                <source src="{{ asset($track->audio_file) }}" type="audio/mpeg">
                            </audio>
                        </div>
                        <div class="flex flex-col gap-2">
                            <a href="{{ route('admin.gallery.music.edit', $track) }}" class="px-4 py-2 rounded-lg bg-blue-100 text-blue-600 hover:bg-blue-200 transition-all text-sm font-semibold text-center">
                                Edit
                            </a>
                            <form action="{{ route('admin.gallery.music.destroy', $track) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus musik ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full px-4 py-2 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 transition-all text-sm font-semibold">
                                    Hapus
                                </button>
                            </form>
                        </div>
